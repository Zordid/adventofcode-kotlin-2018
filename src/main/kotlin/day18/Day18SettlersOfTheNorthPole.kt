...
class LumberArea(puzzle: List<String>) {
    ...
    fun future(min: Int, debug: Boolean = false): List<List<Char>> {
        var map = initialMap
        ...
        while (!seenBefore.contains(map) && minutesLeft > 0) {
            ...
            minutesLeft--
            if (debug) println("After ${min - minutesLeft} minutes:\n${map.neat()}")
        }
        if (minutesLeft == 0)
            return map

        if (debug)
            println("Loop detected after ${min - minutesLeft} minutes with $minutesLeft minutes to go!")
    ...
    fun solve(min: Int, debug: Boolean = false): Int {
        val future = future(min, debug)
    ...
    private fun List<List<Char>>.of(c: Coordinate) =
        if ((c.y >= 0) && (c.y < size) && (c.x >= 0) && (c.x < this[c.y].size)) this[c.y][c.x] else null
